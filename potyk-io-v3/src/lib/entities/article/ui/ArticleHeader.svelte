<script lang="ts">
	import H1 from '$lib/shared/ui/H1.svelte';
	import Breadcrumbs from '$lib/entities/article/ui/Breadcrumbs.svelte';
	import type { Article } from '$lib/entities/article/articles';

	export let article: Article | null = null;
</script>

<div class="flex gap-2 bg-white">
	<div class="text-2xl md:text-5xl font-black">
		<Breadcrumbs />
	</div>
	<div class="flex flex-col md:gap-2">
		<H1>
			<slot>{article?.title}</slot>
		</H1>
		<div class="text-base md:text-xl text-gray-400">
			<slot name="desc">{article?.desc ?? ''}</slot>
		</div>
		<div class="flex">
			<slot name="tags" />
		</div>
	</div>
</div>
