<script lang="ts">
  ...
  let beerTypesGrid: { title: string, desc: string, examples?: string[] }[] = [
    ...
    { title: "Пшеничное / Weiss", desc: "Мутнячок / Светлое нефильтрованное", examples: ["Paulaner"] },
    { title: "Эль", desc: "Ипы всякие, горчинистые", examples: ["Alaska", "Panzer", "Stamm", "Jaws"] },
    ...
    { title: "Медовуха / Mead / Melomel", desc: "Очевидно с медовым вкусом", examples: ["Традиции Предков / mjolnir"] },
...
<div class="grid grid-cols-3 gap-4">
  {#each beerPlaces as beer}
    <a href={beer.link} class="text-xl shadow p-4 rounded-xl border-blue-300 hover:border-blue-500 border-2  flex space-y-2 flex-col justify-center">
      <div class="text-lg font-bold">{beer.title}</div>
    </a>
  {/each}
